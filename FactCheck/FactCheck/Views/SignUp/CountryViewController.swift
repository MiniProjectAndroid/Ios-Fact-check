...
extension CountryViewController : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = "\(self.dupList[indexPath.row].flag!) \(self.dupList[indexPath.row].name!)"
        return cell!
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //let countryname = dupList[indexPath.row].name!
        utilisateur?.countryNameValue = dupList[indexPath.row].name
        performSegue(withIdentifier: "newsources", sender: indexPath)
        
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "newsources"{
        let destination = segue.destination as! NewSourceViewController
...
extension String {
    static func flag(for code : String) -> String? {
        ...
        func regionlIndicatorSymbol(for scalar: Unicode.Scalar) -> Unicode.Scalar {
        ...
        let  indicatorSymbols = lowercaseCode.unicodeScalars.map({ regionlIndicatorSymbol(for: $0)})
