...
class ListViewController: UIViewController ,UITableViewDataSource, UITableViewDelegate{
  ...
  //outlet
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let newsUser = newsUser[indexPath.row]
        let url = URL(string: host + newsUser.image!)!
               if let data = try? Data(contentsOf: url ) {
                   cell.imageNews.image = UIImage(data: data )
               }
            let id = newsUser.username
        ...
        cell.dateNews.text = newsUser.date
        cell.titleNews.text = newsUser.title
        cell.likeNews.text = newsUser.like
        cell.commentaireNews.text = newsUser.comment
