...
class CommentViewController: UIViewController ,UITableViewDataSource, UITableViewDelegate{
    ...
    @objc func longPress(sender: UILongPressGestureRecognizer) {
                ...
                if sender.state == UIGestureRecognizer.State.began {
                    ...
                    if let indexPath = tableViewComment.indexPathForRow(at: touchPoint) {
                        print(posts[indexPath.row]._id)
                    let indexpath1 = IndexPath(row: tableViewComment.tag, section: 0)
                       // print(posts[indexpath1.row]._id!)
                        // your code here, get the row for the indexPath or do whatever you want
                        
                        
                        //print(posts[indexpath1.row].username!)
    ...
    func updatItem(id: String,description:String)
    {
        ...
        postViewModel.updatePost(id: id ,description:description,completed: { [self] (success) in
           // print(description)
           // print(id)
            if success {
                //initialize()
